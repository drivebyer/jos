...
boot_alloc(uint32_t n)
{
	...
	if (n > 0) {
		...
		return result; /*注意虽然分配了物理页，但是应该返回分配前的地址，且返回的是虚拟地址*/
	}
...
mem_init(void)
{
	...
	kern_pgdir = (pde_t *) boot_alloc(PGSIZE); /*分配一页存放Page Directory*/
	...
	/*
	 * 设置Page Directory第0x3BD号entry，指向Page Directory自己
	 * 第0x3BD号entry
	 * 从UVPT开始一共有1024个Page-Table pages映射到这块虚拟地址空间中
	 * UVPT是在memlayout.h中通过宏定义的，所以0x3BD，也是由这个定义的值得到的
	 */
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (*pde & PTE_P)
	{
		...
		pp = page_alloc(1);  /*这里一定要填1，不然分配出来的PTE上可能有脏值*/
		if (!pp) return NULL;
		pp->pp_ref++;
		*pde = page2pa(pp) | PTE_P | PTE_W | PTE_U; /*由于是新加的page table，所以需要设置对应的PDE*/
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
  ...
  // for(;;){
  //   pte = pgdir_walk(pgdir, a, 1); /*walkpgdir()中已经设置好PDE*/
  //   if(*pte & PTE_P)
  //     panic("remap");
  //   /*
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	size = (physaddr_t)ROUNDUP(pa+size, PGSIZE);
	size -= pa;
